...
test("TikTok Ads Adapter (live) - fetchInsights parses metrics from the report endpoint", async () => {
  await withMockFetch(
    (async (url: string) => {
      ...
      return {
        ok: true,
        json: async () => ({ data: { list: [{ metrics: { impressions: "1000", reach: "750", clicks: "40", conversions: "4", spend: "20.00", total_complete_payment: "88.00" } }] } }),
      } as Response;
    }) as typeof fetch,
    async () => {
      const stats = await tiktokAdapter.fetchInsights("tiktok_123456", "2026-07-06");
      // revenueCents = total_complete_payment (88.00) * 100 → true ROAS input.
      assert.deepStrictEqual(stats, { impressions: 1000, reach: 750, clicks: 40, conversions: 4, spendCents: 2000, revenueCents: 8800 });
    }
  );
});
