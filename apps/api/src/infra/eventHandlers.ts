...
import { emitCampaignStatus, emitNotification } from "./realtimeBridge.js";
 ...
 *
 * Each handler also pushes a real-time WebSocket event so connected browsers
...
export function registerEventHandlers(): void {
  eventBus.subscribe<CampaignLaunchedEvent>("campaign.launched", (event) => {
    ...
    // Push real-time status update to connected browsers (businessId is the routing key)
    void emitCampaignStatus(event.payload.businessId, event.payload.campaignId, "active");
    void emitNotification(event.payload.businessId, {
      type: "campaign.launched",
      title: "Campaign Launched",
      message: `Campaign is live with ${event.payload.activeVariants} active variants`,
