  ...
  async execute(input: ResearchProviderInput): Promise<ProviderResult<CompanyData>> {
    return runProviderStep(this.name, 1, async () => {
      ...
      const { status, data, citations } = await webSearchThenStructure<CompanyData>({
        maxTokens: 768,
        tool: COMPANY_TOOL,
        searchPrompt: `Research the company behind ${label}. Find: legal/brand name, what it does, founding year, headquarters location, employee count range, and funding stage/ownership (bootstrapped, VC-backed, public, etc).`,
