...
import { emitNewLead } from "../../infra/realtimeBridge.js";
...
export async function ingestLead(input: IngestLeadInput): Promise<LeadRecord> {
  ...
  if (!existingLead) {
    ...
    void emitNewLead(input.workspaceId, { id: record.id, fullName: record.fullName, email: record.email, platform: record.platform, submittedAt: record.submittedAt });
    return record;
  }
